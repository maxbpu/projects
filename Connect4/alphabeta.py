...
def maxturn(board, lastmove, ops, player, og, depth, a, b, a1, a2, a3):
  bestop = -1
  if terminal(board, og, lastmove, player):
    ...
    for x in OPS:
      ...
      if possible:
        (val1, y) = minturn(board1, x, ops, (1-player), og, depth+1, a, b, a1, a2, a3)
        if val1 > val:
          val = val1
          bestop = x
        elif val1 == val:
...
def minturn(board, lastmove, ops, player, og, depth, a, b, a1, a2, a3):
  bestop = -1
  if terminal(board, og, lastmove, player):
    return (value(board, og, lastmove), lastmove)
  elif depth>5: 
    ...
    for x in OPS:
      (possible, board1) = apply(board, x, player)
      if possible:
        (val1, y) = maxturn(board1, x, ops, (1-player), og, depth+1, a, b, a1, a2, a3)
        if val1 < val:
          val = val1
          bestop = x
        elif val1 == val:
...
def choosebyalphabeta(board, player, lastmove, a1, a2, a3):

  alpha = -math.inf
  beta = math.inf

  (val, bestop) = maxturn(board, lastmove, OPS, player, player, 0, alpha, beta, a1, a2, a3)
  return bestop
